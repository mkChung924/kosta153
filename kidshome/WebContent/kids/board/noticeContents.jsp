	...
	function list() {
		...
		function createReply(reply) {
			...
			$(replyDiv).html("<table border=0 width=1100><tr><td width=120 align='center'>" +
					reply.user_id + ":</td><td width=600>" + reply.content + "</td><td align='center'>" +reply.gdate + "<input type='hidden' id=r" + reply.reply_no + "value='" + reply.reply_no + "'/><c:if test='${dto.user_id eq id}'><td align='center'><a href=\"javascript:callUpForm('" + reply.reply_no + "')\">수정</a> <a href=\"javascript:deleteR('" + reply.reply_no + "')\">삭제</a></td></c:if></tr></table>");
			return replyDiv;
		}
	}
	$(function() {
	...
	<center>
		...
		<form action="boardinput.do?action=selectContent" method="post">
			<table border=0 cellspacing=0 cellpadding=5 width="1100">
				<tr bgcolor='#FAECC5'>
					<td align='center' colspan="3"><font size='3'>${dto.notice_title }</font></td>
				</tr>
				<tr>
					<td align="left" width="366"><font size="2"> 작성자: ${dto.user_id }</font></td>
					<td align="center" width="366"><font size="2">작성일: ${dto.gdate }</font></td>
					<td align="right" width="366"><font size=2>조회수: ${dto.notice_count }</font></td>
				</tr>
			</table>
			<table border="1" cellspacing="0" height="300" width="1100">	
				<tr>
					<td colspan="3" align="center" height="300">${dto.notice_content }</td>
				</tr>
			</table>
		</form>
		<br>
		<div id="comment">안녕</div>
		...
		<br>
		<input type="text" id="commentText" size="100"/> <input type="button" id="commentInput" value="댓글 입력" /> <br>
		<br>
		<form>
			<c:if test="${auth eq 2 }">	
			<a href="updel.do?action=updateGet&no=${dto.no }">
			<input type="button" value="수정하기"></a> 
			<a href="updel.do?action=delete&no=${dto.no }">
