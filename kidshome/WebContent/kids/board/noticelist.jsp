...
<body>
	...
	<center>
		...
		<table border='0' cellspacing=1 cellpadding=2 width='1100'>
			<tr bgcolor='#FFD8D8' align="center">
			...
			<tr bgcolor='ededed'  align="center">
				...
				<td><a href="updel.do?action=call&no=${Board.no }"> <font size=3 color="black">${Board.notice_title }</font></a></td>
				<td><font size=3>${Board.user_id }</font></td>
				<td><font size=3>${Board.gdate }</font></td>
				<td><font size=3>${Board.notice_count }</font></td>
			</tr>
			 </c:forEach> 
		</table>
		<br><br>
    ...
    <br><br>
      <c:choose>
        ...
        <c:otherwise>
         <a href="noticelist.do?page=${page-1 }">이전</a> 
        </c:otherwise>
      </c:choose>
      ...
      <c:if test="${page<totalPage }">
        <a href="noticelist.do?page=${page+1 }">다음</a>
      </c:if>
